...
class coupang_category(crwaling):
    ...
    #     self.datas = self.soup.find('div', {'id': 'optionWrapper'})
    #
    #     self.SingleData = self.datas.find_all(class_='single-attribute__textLabel')
    #     self.DropboxData = self.datas.find_all(class_='Dropdown-Select__Dropdown__Item')
    #     self.ColortextData = self.datas.find_all(class_='Text-Select__Item')
    #     self.ColorimageData = self.datas.find_all(class_='Image-Select__Item')
    #
...
class CoupangItem:
    ...
    def itemData(self):
        ...
        # 현재 페이지의 html 얻기
        self.soup = BeautifulSoup(driver.page_source, 'html.parser')
        # 아이템 정보가 있는 optionWrpper 부분 가져오기
        self.datas = self.soup.find('div', {'id': 'optionWrapper'})
        # 아이템 타이틀 얻기
        self.ItemTitle = self.soup.find(class_='prod-buy-header__title').get_text(strip=True)
